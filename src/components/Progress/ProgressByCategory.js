...
const ProgressByCategory = () =>{
    ...
    useEffect(() => {
        onAuthStateChanged(auth, async (currentUser) => {
          if (currentUser && currentUser.uid) {
            ...
            const loadedData =  dataArray.map(item => {return {data: {labels: Array.from({length: item.data.length}, (_, i) => i),
            average: item.data.reduce((total, value) => total + parseInt(value), 0) / item.data.length,
                datasets: [{ 
    ...
    return <section className='progress-category'>
            {categoryProgress && categoryProgress.map(item => <BarChart key={item.id} data={item.data} />)}
    </section>
}
